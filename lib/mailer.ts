// lib/mailer.ts
// Uses nodemailer. Configure via environment variables:
...
export async function sendMail(subject: string, html: string) {
  if (!process.env.EMAIL_USER) {
    // In dev / preview without creds, just log
    console.log('[MAIL]', subject, html)
    return
  }
...
export function tableHtml(data: Record<string, string>) {
  const rows = Object.entries(data)
    .map(([k, v]) => `<tr><td style="padding:6px 12px;font-weight:600;background:#f5f4f1;border:1px solid #e2ddd5">${k}</td><td style="padding:6px 12px;border:1px solid #e2ddd5">${v || '—'}</td></tr>`)
    .join('')
